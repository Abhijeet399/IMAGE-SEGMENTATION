...
def resize(features,labels):
    for i in range(1464):
        features[i]=cv2.resize(features[i],(512,512))
        labels[i]=cv2.resize(labels[i],(512,512))
    return features,labels
...
def forward_propagation(X, parameters):
    ...
    CONV2D -> RELU -> MAXPOOL -> CONV2D -> RELU -> MAXPOOL -> Then Transform COnvolutional layer    
    Arguments:
    ...
    # RELU
    A3 = tf.nn.relu(Z3)
    Z4 = tf.nn.conv2d_transpose(A3,W4,[253,253,8],[1,3,3,1],padding='SAME')
    A4 = tf.nn.relu(Z4)
    Z5 = tf.nn.conv2d_transpose(A4,W5,[512,512,3],[1,3,3,1],padding='SAME')
    A5 = tf.nn.relu(Z4)
    return A5
...
def compute_cost(A5, Y):
    ...
    Computes the cost
    Arguments:
    A4 -- output of forward propagation (output of the last LINEAR unit), 
    Y -- "Ground true" labels placeholder, same shape as A4
...
def model(X_train, Y_train, X_test, Y_test, learning_rate = 0.009,
    ...
    Arguments:
    X_train -- training set, of shape (None, 512, 512, 3)
